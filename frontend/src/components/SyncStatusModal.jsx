...
/**
 * SyncStatusModal - Sources and sync audit modal with soap bubble styling
 */
export default function SyncStatusModal({ isOpen, onClose, providers, syncStatuses, onTriggerSync, isSyncing }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3.5 sm:p-4 bg-slate-950/60 backdrop-blur-md animate-fadeIn">
      <div className="w-full max-w-2xl max-h-[88vh] overflow-y-auto rounded-[32px] bubble-surface p-5 sm:p-8 relative shadow-2xl text-slate-800 dark:text-slate-200">
        ...
        <div className="space-y-3 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
            {providers && providers.length > 0 ? (
              providers.map((p, idx) => (
                <div key={idx} className="p-3.5 rounded-2xl bubble-pill flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    <span className={`w-2 h-2 rounded-full ${p.available ? 'bg-emerald-500 shadow-sm shadow-emerald-500/50' : 'bg-slate-400'}`} />
                    <span className="font-mono text-xs font-bold text-slate-800 dark:text-slate-200">{p.name}</span>
                  </div>
                  <span className={`text-[10px] uppercase font-bold px-2.5 py-0.5 rounded-full ${
                    p.available ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/80 dark:text-emerald-300' : 'bg-slate-200 text-slate-600 dark:bg-slate-800 dark:text-slate-400'
                  }`}>
        ...
        {/* Sync History Audit */}
        <div className="mb-6">
          <h4 className="text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-300 mb-3">
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
            {syncStatuses && syncStatuses.length > 0 ? (
              syncStatuses.map((status, idx) => (
                <div key={idx} className="p-3 rounded-2xl bubble-pill flex items-center justify-between text-xs shadow-sm">
                  <div className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-500 flex-shrink-0" />
                    <div>
                      <span className="font-bold text-slate-800 dark:text-slate-200">{status.providerName}</span>
                  ...
                  <div className="text-right">
                    <span className="text-emerald-600 dark:text-emerald-400 font-mono font-semibold">+{status.newlyAddedTracks || 0} pistas</span>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 block">{status.totalTracksSynced || 0} total</span>
                  </div>
                </div>
              ))
            ) : (
        ...
        {/* Footer Actions */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-4 border-t border-slate-200/50 dark:border-slate-800">
          ...
          <button
            ...
            disabled={isSyncing}
            className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-full bubble-surface bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold shadow-md transition active:scale-95 disabled:opacity-50 cursor-pointer"
          >
